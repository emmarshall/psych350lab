...
# Integrates with existing package calculation functions (chi_square_answers,
# corr_answers, bg_anova_answers, regression_answers).
# =============================================================================
...
spss_descriptives_html <- function(data, variables, digits = 2) {
  ...
  for (var in variables) {
    ...
    rows_html <- paste0(rows_html, sprintf(
      '<tr><td class="label">%s</td><td>%d</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>\n',
  ...
  # Valid N row
  rows_html <- paste0(rows_html, sprintf(
    '<tr><td class="label">Valid N (listwise)</td><td>%d</td><td></td><td></td><td></td><td></td><td></td></tr>\n',
    valid_n
  ))
...
spss_descriptives_png <- function(data, variables, filename = "descriptives.png",
  ...
  for (var in variables) {
    ...
    tbl_data <- rbind(tbl_data, data.frame(
      ...
      N = length(x),
      Mean = mean(x),
      `Std. Deviation` = stats::sd(x),
      `Std. Error` = stats::sd(x) / sqrt(length(x)),
      Minimum = min(x),
      Maximum = max(x),
      check.names = FALSE,
      stringsAsFactors = FALSE
  ...
  gt_tbl <- gt::gt(tbl_data) |>
    ...
    gt::fmt_number(columns = c("Mean", "Std. Deviation", "Std. Error", "Minimum", "Maximum"),
                   decimals = digits) |>
...
spss_descriptives_by_group_html <- function(data, dv, iv, data_raw = NULL,
  ...
  for (g in names(groups)) {
    ...
    ci <- if (n > 1) {
      tryCatch(stats::t.test(x, conf.level = conf.level)$conf.int, error = function(e) c(NA, NA))
    } else c(NA, NA)

    label <- if (!is.null(val_labels) && g %in% names(val_labels)) val_labels[[g]] else g

    rows_html <- paste0(rows_html, sprintf(
      '<tr><td class="label">%s</td><td>%d</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>\n',
  ...
  rows_html <- paste0(rows_html, sprintf(
    '<tr><td class="label">Total</td><td>%d</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>\n',
  ...
  html <- paste0(
    .spss_table_css(),
    '<table class="spss-table">\n',
    '<caption>Descriptives: ', htmltools::htmlEscape(dv), '</caption>\n',
    '<thead>\n',
    '<tr class="header-row"><th></th><th></th><th></th><th></th><th></th><th colspan="2">', ci_pct, '</th><th></th><th></th></tr>\n',
    '<tr><th></th><th>N</th><th>Mean</th><th>Std. Deviation</th><th>Std. Error</th><th>Lower Bound</th><th>Upper Bound</th><th>Minimum</th><th>Maximum</th></tr>\n',
    '</thead>\n',
    '<tbody>\n',
    rows_html,
    '</tbody>\n',
...
#' result <- bg_anova_answers(superman, "clark_grp", "rt_critics_score")
#' spss_anova_html(result, dv = "rt_critics_score")
#'
#' @export
spss_anova_html <- function(data, dv = NULL, iv = NULL, digits = 3) {

  # Check if data is already anova results
  if (is.list(data) && "ANOVA" %in% names(data)) {
    anova_results <- data
    if (is.null(dv)) dv <- "Dependent Variable"
  } else {
    if (is.null(dv) || is.null(iv)) {
      stop("Must provide 'dv' and 'iv' when passing raw data")
    }
    # Compute ANOVA
  ...
  # If ss values not available, try to compute from existing data
  if (is.na(ss_b) && !is.null(ms_b) && !is.null(a$df_between)) {
    ss_b <- ms_b * a$df_between
  }
  if (is.na(ss_w) && !is.null(ms_w) && !is.null(a$df_within)) {
    ss_w <- ms_w * a$df_within
  }
  ...
  html <- paste0(
    .spss_table_css(),
    '<table class="spss-table">\n',
    '<caption>ANOVA: ', htmltools::htmlEscape(dv), '</caption>\n',
    '<thead>\n',
    '<tr><th></th><th>Sum of Squares</th><th>df</th><th>Mean Square</th><th>F</th><th>Sig.</th></tr>\n',
    '</thead>\n',
    '<tbody>\n',
    sprintf('<tr><td class="label">Between Groups</td><td>%s</td><td>%d</td><td>%s</td><td>%s</td><td>%s</td></tr>\n',
            .spss_fmt(ss_b, digits), a$df_between, .spss_fmt(ms_b, digits),
            .spss_fmt(a$F, digits), .spss_fmt_p(a$p_value, digits)),
    sprintf('<tr><td class="label">Within Groups</td><td>%s</td><td>%d</td><td>%s</td><td></td><td></td></tr>\n',
            .spss_fmt(ss_w, digits), a$df_within, .spss_fmt(ms_w, digits)),
    sprintf('<tr><td class="label">Total</td><td>%s</td><td>%d</td><td></td><td></td><td></td></tr>\n',
...
spss_levene_html <- function(data, dv, iv, digits = 3) {
  ...
  html <- paste0(
    .spss_table_css(),
    '<table class="spss-table">\n',
    '<caption>Test of Homogeneity of Variances: ', htmltools::htmlEscape(dv), '</caption>\n',
    '<thead>\n',
...
#'
#' Creates an HTML chi-square test table from chi_square_answers() output
#' or raw data.
#'
...
#' spss_chisq_html(result)
#'
#' @export
spss_chisq_html <- function(data, var1 = NULL, var2 = NULL, digits = 3) {

  # Check if data is already chi-square results
  if (is.list(data) && "ChiSquare" %in% names(data)) {
    ...
    if (is.null(var1) || is.null(var2)) {
      stop("Must provide 'var1' and 'var2' when passing raw data")
    }
  ...
  rows_html <- sprintf(
    '<tr><td class="label">Pearson Chi-Square</td><td>%s</td><td>%d</td><td>%s</td></tr>\n',
  ...
  if (is_2x2) {
    ...
    rows_html <- paste0(rows_html, sprintf(
      '<tr><td class="label">Continuity Correction</td><td>%s</td><td>%d</td><td>%s</td></tr>\n',
      .spss_fmt(chi_corrected$statistic, digits), chi_corrected$parameter,
      .spss_fmt_p(chi_corrected$p.value, digits)
    ))
  }

  rows_html <- paste0(rows_html, sprintf(
    '<tr><td class="label">N of Valid Cases</td><td>%d</td><td></td><td></td></tr>\n', n
  ))

  footnote1 <- sprintf("%d cells (%.1f%%) have expected count less than 5. The minimum expected count is %.2f.",
                       low_expected, pct_low, min_expected)
  footnote2 <- if (is_2x2) "Computed only for a 2x2 table." else ""
  ...
  html <- paste0(
    ...
    '</tbody>\n',
    '<tfoot>\n',
    '<tr><td colspan="4">', footnote1, '</td></tr>\n',
    if (nchar(footnote2) > 0) paste0('<tr><td colspan="4">', footnote2, '</td></tr>\n') else "",
    '</tfoot>\n',
    '</table>\n'
  )
...
#'
#' @export
spss_crosstab_html <- function(data, var1 = NULL, var2 = NULL,
                               data_raw = NULL, digits = 1) {
  ...
  for (i in seq_len(nr)) {
    ...
    # Count row
    rows_html <- paste0(rows_html, sprintf(
      '<tr><td class="label" rowspan="2">%s</td><td class="label">Count</td>',
      htmltools::htmlEscape(row_label)
    ))
    for (j in seq_len(nc)) {
      rows_html <- paste0(rows_html, sprintf('<td>%d</td>', tab[i, j]))
    }
    rows_html <- paste0(rows_html, sprintf('<td>%d</td></tr>\n', sum(tab[i, ])))

    # Expected row
    rows_html <- paste0(rows_html, '<tr><td class="label">Expected Count</td>')
    for (j in seq_len(nc)) {
      rows_html <- paste0(rows_html, sprintf('<td>%s</td>', .spss_fmt(expected[i, j], digits)))
    }
  ...
  rows_html <- paste0(rows_html, '<tr><td class="label" rowspan="2">Total</td><td class="label">Count</td>')
  for (j in seq_len(nc)) {
    rows_html <- paste0(rows_html, sprintf('<td>%d</td>', col_totals[j]))
  }
  rows_html <- paste0(rows_html, sprintf('<td>%d</td></tr>\n', sum(col_totals)))

  rows_html <- paste0(rows_html, '<tr><td class="label">Expected Count</td>')
  for (j in seq_len(nc)) {
    rows_html <- paste0(rows_html, sprintf('<td>%s</td>', .spss_fmt(exp_totals[j], digits)))
  }
  ...
  html <- paste0(
    .spss_table_css(),
    '<table class="spss-table">\n',
    '<caption>', htmltools::htmlEscape(var1), ' * ', htmltools::htmlEscape(var2), ' Crosstabulation</caption>\n',
    '<thead>\n',
    header_html,
    '</thead>\n',
    '<tbody>\n',
...
spss_correlations_html <- function(data, variables, digits = 3) {
  ...
  # Build header
  header_html <- paste0('<tr><th></th><th></th>',
                        paste(sapply(variables, function(v) paste0('<th>', htmltools::htmlEscape(v), '</th>')), collapse = ""),
                        '</tr>\n')
  ...
  for (i in seq_len(nvars)) {
    ...
    rows_html <- paste0(rows_html, sprintf(
      '<tr><td class="label" rowspan="3">%s</td><td class="label">Pearson Correlation</td>',
      htmltools::htmlEscape(variables[i])
    ))
    for (j in seq_len(nvars)) {
      r <- cor_mat[i, j]
      star <- ""
      ...
      rows_html <- paste0(rows_html, sprintf('<td>%s%s</td>', .spss_fmt(r, digits), star))
    }
    rows_html <- paste0(rows_html, '</tr>\n')

    # Sig row
    rows_html <- paste0(rows_html, '<tr><td class="label">Sig. (2-tailed)</td>')
    for (j in seq_len(nvars)) {
      if (i == j) {
        rows_html <- paste0(rows_html, '<td></td>')
      } else {
        rows_html <- paste0(rows_html, sprintf('<td>%s</td>', .spss_fmt_p(p_mat[i, j], digits)))
      }
    }
    rows_html <- paste0(rows_html, '</tr>\n')

    # N row
    rows_html <- paste0(rows_html, '<tr><td class="label">N</td>')
    for (j in seq_len(nvars)) {
      rows_html <- paste0(rows_html, sprintf('<td>%d</td>', n))
    }
    rows_html <- paste0(rows_html, '</tr>\n')
  }
  ...
  html <- paste0(
    ...
    '</tbody>\n',
    '<tfoot>\n',
    '<tr><td colspan="', nvars + 2, '">**. Correlation is significant at the 0.01 level (2-tailed).</td></tr>\n',
    '<tr><td colspan="', nvars + 2, '">*. Correlation is significant at the 0.05 level (2-tailed).</td></tr>\n',
    '</tfoot>\n',
    '</table>\n'
  )
...
#' @examples
#' data(superman)
#' sm <- superman[complete.cases(superman[, c("rt_critics_score", "clark_height_in")]), ]
#' result <- regression_answers(sm, "rt_critics_score",
#'                              quant_predictors = "clark_height_in")
#' spss_regression_model_html(result)
#'
#' @export
spss_regression_model_html <- function(reg_results, digits = 3) {
  ...
  html <- paste0(
    ...
    '</thead>\n',
    '<tbody>\n',
    sprintf('<tr><td>1</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>\n',
            .spss_fmt(m$R, digits), .spss_fmt(m$R_squared, digits),
            .spss_fmt(m$Adj_R_squared, digits),
...
spss_regression_anova_html <- function(reg_results, digits = 3) {
  ...
  # Get ANOVA table from raw model
  ...
  html <- paste0(
    ...
    '<thead>\n',
    '<tr><th>Model</th><th>Sum of Squares</th><th>df</th><th>Mean Square</th><th>F</th><th>Sig.</th></tr>\n',
    '</thead>\n',
    '<tbody>\n',
    sprintf('<tr><td class="label">Regression</td><td>%s</td><td>%d</td><td>%s</td><td>%s</td><td>%s</td></tr>\n',
            .spss_fmt(ss_reg, digits), df_reg, .spss_fmt(ms_reg, digits),
            .spss_fmt(m$F, digits), .spss_fmt_p(m$p_value, digits)),
    sprintf('<tr><td class="label">Residual</td><td>%s</td><td>%d</td><td>%s</td><td></td><td></td></tr>\n',
...
spss_regression_coefficients_html <- function(reg_results, digits = 3) {
  ...
  for (i in seq_len(nrow(coefs))) {
    ...
    rows_html <- paste0(rows_html, sprintf(
      '<tr><td class="label">%s</td><td>%s</td><td>%s</td><td></td><td>%s</td><td>%s</td></tr>\n',
      htmltools::htmlEscape(name),
      .spss_fmt(coefs[i, "Estimate"], digits),
      .spss_fmt(coefs[i, "Std. Error"], digits),
      .spss_fmt(coefs[i, "t value"], digits),
      .spss_fmt_p(coefs[i, "Pr(>|t|)"], digits)
    ))
  }
  ...
  html <- paste0(
    ...
    '<thead>\n',
    '<tr class="header-row"><th></th><th colspan="2">Unstandardized Coefficients</th><th>Standardized Coefficients</th><th></th><th></th></tr>\n',
...
spss_ttest_independent_html <- function(data, dv, iv, digits = 3) {
  ...
  html <- paste0(
    .spss_table_css(),
    '<table class="spss-table">\n',
    '<caption>Independent Samples Test</caption>\n',
    '<thead>\n',
    '<tr class="header-row"><th></th><th></th><th colspan="2">Levene\'s Test</th><th colspan="7">t-test for Equality of Means</th></tr>\n',
    '<tr><th></th><th></th><th>F</th><th>Sig.</th><th>t</th><th>df</th><th>Sig. (2-tailed)</th><th>Mean Difference</th><th>Std. Error Difference</th><th colspan="2">95% CI</th></tr>\n',
    '</thead>\n',
    '<tbody>\n',
    sprintf('<tr><td class="label" rowspan="2">%s</td><td class="label">Equal variances assumed</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td colspan="2">%s to %s</td></tr>\n',
            htmltools::htmlEscape(dv),
            .spss_fmt(lev$statistic, digits), .spss_fmt_p(lev$p.value, digits),
            .spss_fmt(t_eq$statistic, digits), .spss_fmt(t_eq$parameter, 0),
            .spss_fmt_p(t_eq$p.value, digits), .spss_fmt(md, digits),
            .spss_fmt(t_eq$stderr, digits),
            .spss_fmt(t_eq$conf.int[1], digits), .spss_fmt(t_eq$conf.int[2], digits)),
    sprintf('<tr><td class="label">Equal variances not assumed</td><td></td><td></td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td colspan="2">%s to %s</td></tr>\n',
            .spss_fmt(t_welch$statistic, digits), .spss_fmt(t_welch$parameter, digits),
            .spss_fmt_p(t_welch$p.value, digits), .spss_fmt(md, digits),
            .spss_fmt(t_welch$stderr, digits),
            .spss_fmt(t_welch$conf.int[1], digits), .spss_fmt(t_welch$conf.int[2], digits)),
    '</tbody>\n',
    '</table>\n'
  )
...
spss_ttest_paired_html <- function(data, var1, var2, digits = 3) {
  ...
  html <- paste0(
    .spss_table_css(),
    '<table class="spss-table">\n',
    '<caption>Paired Samples Test</caption>\n',
    '<thead>\n',
    '<tr><th></th><th>Mean</th><th>Std. Deviation</th><th>Std. Error Mean</th><th>t</th><th>df</th><th>Sig. (2-tailed)</th></tr>\n',
    '</thead>\n',
    '<tbody>\n',
    sprintf('<tr><td class="label">%s - %s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%d</td><td>%s</td></tr>\n',
            htmltools::htmlEscape(var1), htmltools::htmlEscape(var2),
            .spss_fmt(mean(d), digits), .spss_fmt(stats::sd(d), digits),
            .spss_fmt(stats::sd(d) / sqrt(length(d)), digits),
            .spss_fmt(tobj$statistic, digits), tobj$parameter,
            .spss_fmt_p(tobj$p.value, digits)),
    '</tbody>\n',
    '</table>\n'
  )
...
spss_ttest_one_html <- function(data, variable, mu = 0, digits = 3) {
  ...
  html <- paste0(
    .spss_table_css(),
    '<table class="spss-table">\n',
    '<caption>One-Sample Test (Test Value = ', mu, ')</caption>\n',
    '<thead>\n',
    '<tr class="header-row"><th></th><th></th><th></th><th></th><th colspan="2">95% Confidence Interval of the Difference</th></tr>\n',
    '<tr><th></th><th>t</th><th>df</th><th>Sig. (2-tailed)</th><th>Lower</th><th>Upper</th></tr>\n',
    '</thead>\n',
    '<tbody>\n',
    sprintf('<tr><td class="label">%s</td><td>%s</td><td>%d</td><td>%s</td><td>%s</td><td>%s</td></tr>\n',
            htmltools::htmlEscape(variable),
            .spss_fmt(tobj$statistic, digits), tobj$parameter,
            .spss_fmt_p(tobj$p.value, digits),
            .spss_fmt(tobj$conf.int[1] - mu, digits),
            .spss_fmt(tobj$conf.int[2] - mu, digits)),
    '</tbody>\n',
    '</table>\n'
  )
...
#'
#' Creates a histogram with SPSS styling, optional normal curve overlay,
#' and descriptive statistics annotation.
#'
#' @param data A data frame.
...
#' @param fill_color Character. Bar fill color. Default "#1192E8" (SPSS blue).
...
#'
#' @export
spss_histogram <- function(data, variable, data_raw = NULL,
                           bins = 10, show_normal = TRUE, show_stats = TRUE,
  ...
  p <- p +
    theme_SPSS(version = version, scales = "continuous") +
  ...
  if (show_stats) {
    ...
    stats_label <- paste0(
      "Mean = ", formatC(m, digits = 2, format = "f"), "\n",
...
#'
#' @export
spss_barchart <- function(data, variable, data_raw = NULL,
                          show_counts = TRUE, fill_color = "#1192E8",
                          version = "modern") {
  ...
  if (!is.null(val_labels)) {
    freq_df$label <- sapply(as.character(freq_df$category), function(v) {
      if (v %in% names(val_labels)) val_labels[[v]] else v
    })
  } else {
  ...
  p <- p +
    theme_SPSS(version = version, scale.x = "discrete", scale.y = "continuous") +
    ggplot2::labs(x = var_label, y = "Frequency") +
...
#'
#' Saves a ggplot2 plot (such as from spss_histogram or spss_barchart) as PNG.
#'
